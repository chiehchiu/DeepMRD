...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(self, data_prefix, pipeline, ann_file=None, sub_set=None, test_mode=False, use_sid_sampler=False):
        super().__init__()
    ...
    def evaluate(self,
        ...
        if metric == 'accuracy':
            ...
            #LJ 源码此处有bug 此时results已经经过softmax 不能使用F.cross_entropy()
            # loss = cross_entropy(torch.Tensor(results), torch.Tensor(gt_labels).long()).mean()
            loss = F.nll_loss(torch.log(torch.Tensor(results) + 1e-5), torch.Tensor(gt_labels).long()).mean()
