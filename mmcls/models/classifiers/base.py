...
class BaseClassifier(nn.Module, metaclass=ABCMeta):
    ...
    def forward_test(self, imgs, **kwargs):
        ...
        if len(imgs) == 1:
            # LJ add multicrop_test & multipath_test
            if imgs[0].shape[1] != 1 and imgs[0].dim() == 5:
                # return self.multicrop_test(imgs[0], **kwargs)
                return self.multipatch_test(imgs[0], **kwargs)
            else:
