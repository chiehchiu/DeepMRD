...
class ResNet(BaseBackbone):
    ...
    def init_weights(self, pretrained=None):
        ...
        if 'resnet50.pt' in str(pretrained):
            checkpoint = torch.load(pretrained, map_location=torch.device('cpu'))
            if 'state_dict' in checkpoint:
                state_dict = checkpoint['state_dict']
            else:
                state_dict = checkpoint
            # strip prefix of state_dict  module.encoder.
            if list(state_dict.keys())[0].startswith('module.encoder.'):
    ...
    def forward(self, x):
        if self.deep_stem:
            ...
            x = self.norm1(x)
            x = self.relu(x)
        if self.stem_stride and self.stem_pool:  #
            x = self.maxpool(x)
        outs = []
